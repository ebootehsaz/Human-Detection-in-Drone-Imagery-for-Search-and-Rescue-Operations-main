...
// 
//  Frame capture delegate callbacks, session management, and camera initialization are all contained in this class.
//  It controls the capture session, sets up video input and output, and dynamically chooses the best camera device available. 
//  Additionally, it offers ways to begin and end video recording and sends captured frames to a delegate that uses the VideoCaptureDelegate protocol.
//  The VideoCapture class is used by the CameraViewController to capture video frames from the camera and send them to the YOLO model for object detection.
